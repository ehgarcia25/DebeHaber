...
<div class="dataTables_length table-responsive" id="example2_length">
            <table id="example2" class="display table dataTable" style="width: 100%;" role="grid" aria-describedby="example2_info">
               <thead>
                    <tr role="row">
                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column ascending" style="width: 259px;">Cliente</th>
                        ...
                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Acciones: activate to sort column ascending" style="width: 79px;">Acciones</th>
                    </tr>
                </thead>
                {{--<tfoot>--}}
                    {{--<tr>--}}
            ...
            <tbody>
                ...
                <tr role="row" class="odd">
                        <td><a href="{{url('edit_venta',$item->id)}}">{{$item->name}}</a></td>
                        <td>{{$item->invoice_total}}</td>
                    <td>{{$item->invoice_date}}</td>
                    <td>{{App\Divisas::Moneda($item->currency_id)[0]->name}}</td>
                    <td>
                        @if($item->is_accounted_supplier)
                            Aprobada
                        @else
                            Pendiente
                        @endif
                    ...
                    <td>
                        <a href="{{url('delete_venta', $item->id)}}"> <i class="glyphicon glyphicon-trash"></i></a>

                        {!! Form::open([
                        'method'=>'GET',
                        'url' => ['delete_venta', $item->id],
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a class="btn btn-primary btn-lg " href="{{url('ventas_form')}}">Aceptar</a>
